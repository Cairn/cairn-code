    ...
    pub fn new(version: &str, model: &str, provider: &str, work_dir: &str) -> Self {
        Tui {
            ...
            cmd_picker_list: vec![
                ...
                "/exit".into(), "/help".into(), "/mcp".into(), "/model".into(), "/provider".into(),
                "/quit".into(), "/copy".into(), "/q".into(), "/resume".into(), "/save".into(),
                "/select".into(), "/sessions".into(), "/skills".into(), "/suggestions".into(),
                "/theme".into(), "/thinking".into(), "/mouse".into(),
            ],
    ...
    fn handle_command(&mut self, cmd: &str) {
        ...
        match parts[0] {
            ...
            "/skills" => {
                ...
                let dir = crate::skills::default_skills_dir();
                if list.is_empty() {
                    self.output_lines.push(OutputLine {
                        type_: "system".into(),
                        content: format!(
            ...
            "/mcp" => {
                ...
                if cfg.mcp.servers.is_empty() {
                    self.output_lines.push(OutputLine {
                        type_: "system".into(),
                        content: "No MCP servers in config. Add mcp.servers (or mcpServers) to ~/.config/cairn-code/config.json.".into(),
                        tool_name: String::new(),
                        duration: String::new(),
                    });
                } else {
