...
| `/subagent` | List external harnesses, or run one headlessly: `/subagent [worktree\|none] <harness> <prompt…>` (builtins: `claude`, `agy`, `grok`, `zero`). Default isolation is a git worktree under `.cairn/worktrees/`. Also the `subagent` tool. Disable with `subagents.enabled: false` or `CAIRN_SUBAGENTS=0` |
...
| `subagents.default_isolation` | string | `"worktree"` | `worktree` creates `.cairn/worktrees/<id>` + branch `cairn/subagent-<id>`; `none` uses the parent workspace. |
