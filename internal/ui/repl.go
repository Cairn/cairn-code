...
Keyboard shortcuts:
  ...
  Mouse wheel        Scroll output`})
	...
	case "/tools":
		m.output = append(m.output, OutputLine{Type: "system", Content: "Available tools: file_read, file_write, file_edit, bash, glob, grep, go, git, todo_write, web_search, web_fetch"})
		m.markDirty()
		m.ensureContent()
		m.vp.GotoBottom()
		return m, nil
	...
	case "go":
		if m, ok := input.(map[string]any); ok {
			if args, ok := m["args"].([]any); ok {
				...
				cmd := strings.Join(strArgs, " ")
				if len(cmd) > 80 {
					cmd = cmd[:80] + "..."
				}
				return fmt.Sprintf("go %s", cmd)
			}
		}
	case "git":
		if m, ok := input.(map[string]any); ok {
